...
def send_message():  # 카톡에 메세지를 보내는 코드를 send_message()함수로 만듬
    ...
    # -> pyautogui.position(): 함수를 이용해서 채팅창의 위치를 알아온다.
    ...
    pyautogui.hotkey("ctrl", "v") # hotkey: 누르고 싶은 키를 순서대로 입력하면 키를 순서대로 눌러준다.
    time.sleep(1.0)
    # 문구를 넣은 후 1초 딜레이 -> 딜레이를 거는 이유: 너무 빠르면 인식이 잘 안될 수 있으니까 타임 모듈로 딜레이
